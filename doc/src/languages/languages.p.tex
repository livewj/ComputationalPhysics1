...
language. The executable file is in this case \code{myprogram}. The option
\code{-c} is for compilation only, where the program is translated into machine code,
while the \code{-o} option links the produced object file \code{myprogram.o}
and produces the executable \code{myprogram} .

For Fortran2008 we use the Intel compiler, replace \code{c++} with \code{ifort}.
...
${PROG}.o :       ${PROG}.c
                  ${CC} -c ${PROG}.c
\ecppcod
If you name your file for \code{makefile}, simply type the command
\code{make} and Linux/Unix executes all of the statements in the above
makefile. Note that C++ files have the extension \code{.cpp}.
...
\bcppcod
   int main (int argc, char* argv[])
\ecppcod
The integer \code{argc} is the no of command-line arguments, set to
one in our case, while
\code{argv} is a vector of strings containing the command-line arguments
with \code{argv[0]} containing  the name of the program
and \code{argv[1]}, \code{argv[2]}, ... are the command-line args, i.e., the number of
lines of input to the program.
Here we define floating points, see also below,
through the keywords \code{float} for single precision real numbers and
\code{double} for double precision. The function
\code{atof} transforms a text (\code{argv[1]}) to a float.
...
With the command \code{printf} we obtain a formatted printout.
The \code{printf} syntax is used for formatting output
...
\begin{paragraphadmon}[Dissection I.]
We have replaced the call to \code{printf} with the standard C++ function
\code{cout}. The header file \code{<iostream.h>} is then needed.
In addition, we don't need to
declare variables like \code{r} and \code{s}  at the beginning of the program.
I personally prefer
...
\begin{itemize}
  ...
  \item Standard \code{if}, \code{while} and \code{for} statements as in Java, Fortran, Python...
...
\begin{paragraphadmon}[Integer numbers.]
\bcppcod
using namespace std;
#include <iostream>
int main (int argc, char* argv[])
...
\begin{paragraphadmon}[Integer numbers, Fortran.]
\bfcod
...
\begin{paragraphadmon}[Possible Overflow for Integers.]
\bcppcod
// A comment line begins like this in C++ programs
...
\begin{itemize}
  \item \textbf{Overflow}: When the positive exponent exceeds the max value, e.g., 308 for \code{DOUBLE PRECISION} (64 bits). Under such circumstances the program will terminate and some compilers may give you the warning \code{OVERFLOW}.

  \item \textbf{Underflow}: When the negative exponent becomes smaller than the min value, e.g., -308 for \code{DOUBLE PRECISION}. Normally, the variable is then set to zero and the program continues. Other compilers (or compiler options) may warn you with the \code{UNDERFLOW} message and the program terminates.
\end{itemize}
...
\begin{paragraphadmon}[Three ways of computing $e^{-x}$.]
Brute force:
...
\begin{paragraphadmon}[Brute Force.]
\bcppcod
...
\begin{paragraphadmon}[Still Brute Force.]
\bcppcod
int main()
{
   int   n;
...
\begin{paragraphadmon}[Oh, it never ends!]
\bcppcod
...
\begin{paragraphadmon}[Last statements.]
\bcppcod
     while(fabs(term) > TRUNCATION)
       {
	 term *= -x/((double) n);
...
\begin{itemize}
  \item Lines 1,2: Declaration of two variables \code{a} and \code{b}. The compiler reserves two locations in memory. The size of the location depends on the type of variable. Two properties are important for these locations: the address in memory and the content in the location. The value of \code{a} is \code{a}. The address of \code{a} is \code{&a}. The value of \code{b} is \code{*b}. The address of \code{b} is \code{&b}.

  \item Line 3: The value of \code{a} is now 10.
  ...
  \item Line 5: All 10 elements of \code{b} are given values: \code{b[0] = 0}, \code{b[1] = 1}, ....., \code{b[9] = 9}

  \item line 7: here we deallocate the variable \code{b}.
\end{itemize}
...
\begin{itemize}
  \item Line 6: The \code{main()} function calls the function \code{func()} and the program counter transfers to the first statement in \code{func()}. With respect to data the following happens. The content of \code{a} (= 10) and the content of \code{b} (a memory address) are copied to a stack (new memory location) associated with the function \code{func()}

  \item Line 7: The variable \code{x} and \code{y} are local variables in \code{func()}. They have the values  \code{x = 10}, \code{y} is the address of the first element in \code{b} in \code{main()}.

  \item Line 8: The local variable \code{x} stored in the stack memory is changed to 17. Nothing happens with the value \code{a} in \code{main()}.
\end{itemize}
...
\begin{itemize}
  \item Line 9: The value of y is an address and the symbol \code{*y} means the position in memory which has this address. The value in this location is now increased by 10. This means that the value of \code{b[0]} in the main program is equal to 10. Thus func() has modified a value in \code{main()}.

  \item Line 10: This statement has the same effect as line 9 except that it modifies the element \code{b[6]} in \code{main()} by adding a value of 10 to what was there originally, namely 5.

  \item Line 11: The program counter returns to \code{main()}, the next expression after \code{func(a,b)}. All data on the stack associated with \code{func()} are destroyed.
\end{itemize}
...
\begin{itemize}
  \item The value of a is transferred to \code{func()} and stored in a new memory location called \code{x}. Any modification of \code{x} in \code{func()} does not affect in any way the value of \code{a} in \code{main()}. This is called \emph{transfer of data by value}.

  \item On the other hand the next argument in \code{func()} is an address which is transferred to \code{func()}. This address can be used to modify the corresponding value in \code{main()}. In the C language it is expressed as a modification of the value which \code{y} points to, namely the first element of \code{b}. This is called \emph{transfer of data by reference} and is a method to transfer data back to the calling function, in this case \code{main()}.
\end{itemize}
...
In Fortran we can use \code{INTENT(IN)}, \code{INTENT(OUT)}, \code{INTENT(INOUT)} to let the
...
\code{REAL(KIND=8) ::  vector(-10:10)}. Similarly, if we want to start at zero and end at 10 we could write
\code{REAL(KIND=8) ::  vector(0:10)}.
We have also seen that Fortran  allows us to write a matrix addition ${\bf A} = {\bf B}+{\bf C}$ as
\code{A = B + C}.  This means that we have overloaded the addition operator so that it translates this operation into
...
class are written.  The files \code{complex.h} and \code{complex.cpp} are normally
...
The class is defined via the statement \code{class Complex}. We must first use the key word
\code{class}, which in turn is followed by the user-defined variable name  \code{Complex}.
The body of the class, data and functions, is encapsulated  within the parentheses \code{{...}}.

% !split
\subsection{Programming classes}

Data and specific functions can be private, which means that they cannot be accessed from outside the class.
This means also that access cannot be inherited by other functions outside the class. If we use \code{protected}
instead of \code{private}, then data and functions can be inherited outside the class.

% !split
\subsection{Programming classes}

The key word \code{public} means  that data and functions can be accessed from outside the class.
Here we have defined several functions  which can be accessed by functions outside the class.
The declaration \code{friend} means that stand-alone functions can work on privately declared  variables  of the type
\code{(re, im)}.  Data members of a class should be declared as private variables.

% !split
\subsection{Programming classes}

The first public function we encounter is a so-called
constructor, which  tells how we declare a variable of type \code{Complex}
and how this variable is initialized. We have chose  three possibilities in the example above:

A declaration like \code{Complex c;} calls the member function \code{Complex()} which can have the following implementation
...
A call like \code{Complex a(0.1,1.3);} means that we could call the member function `Complex(double, double)`as
...
\ecppcod
Note that we have introduced   the declaration  \code{const}.  What does it mean?
...
\code{const double p = 3;} and then try to change its value, we will get an error when we
...
since \code{Complex::abs} is not a constant member function.

% !split
\subsection{Programming classes}

Constant functions cannot change the object's state.
To avoid this we declare the function \code{abs} as
...
like to write this statement as \code{c = a+b;} as this makes the code much
...
In our main function we have a statement like \code{d = b;}, which means
that we call \code{d.operator= (b)} and we have defined a so-called assignment operator
...
\code{Complex d = b;} or \code{Complex d(b);}
...
{ *this = c; }
\ecppcod
which  is a pointer to "this object", \code{*this} is the present object,
so \code{*this = c;} means setting the present object equal to $c$, that is
\code{this->operator= (c);}.
...
function
\code{Complex operator+ (const Complex& a, const Complex& b); // a+b}
The compiler translates \code{c = a + b;} into \code{c = operator+ (a, b);}.
...
Consider  the case \code{c = a + b;}
that is,  \code{c.operator= (operator+ (a,b));}
If \code{operator+}, \code{operator=} and the constructor \code{Complex(r,i)} all
...
The stand-alone function \code{operator+} is a friend of the Complex  class
...
To inline the complete expression \code{a*b;}, the constructors and
\code{operator=}  must also be inlined.  This can be achieved via the following piece of code
...
\ecppcod
The definitions \code{operator-} and \code{operator/} follow the same set up.

% !split
\subsection{Programming classes}

Finally, if we wish to write to file or another device a complex number using the simple syntax
\code{cout << c;}, we obtain this by defining
...
A simple way to achieve this is copy and paste our class and replace \code{double} with for
example \code{int}.
...
of the keyword \code{template} followed by a list of template arguments enclosed in brackets.
...
What it says is that \code{Complex} is a parameterized type with $T$ as a parameter and $T$ has to be a type such as double
or float.
...
Member functions of our class are defined by preceding the name of the function with the \code{template} keyword.
Consider the function we defined as \code{Complex:: Complex (double re_a, double im_a)}.
...
\begin{paragraphadmon}[What should it contain? A typical structure.]
\begin{itemize}
...
\begin{paragraphadmon}[What should I focus on? References.]
\begin{itemize}
